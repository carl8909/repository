//
//  KFLoginViewController.m
//  online-iOS
//
//  Created by wuy on 2021/2/5.
//  Copyright © 2021 深圳市看房网科技有限公司. All rights reserved.
//

#import "KFLoginViewController.h"


@interface KFLoginViewController ()

@property (nonatomic, copy) NSString *tit;

@property (nonatomic, copy) NSString *tit1;

@property (nonatomic, copy) NSString *tit2;

@property (nonatomic, copy) NSString *tit3;

@property (nonatomic, copy) NSString *tit4;

@property (nonatomic, copy) NSString *tit5;

@end


@implementation KFLoginViewController

- (void)viewDidLoad
{
    [super viewDidLoad];
    // Do any additional setup after loading the view.
}

/*
#pragma mark - Navigation

// In a storyboard-based application, you will often want to do a little preparation before navigation
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender {
    // Get the new view controller using [segue destinationViewController].
    // Pass the selected object to the new view controller.
}
*/

@end
